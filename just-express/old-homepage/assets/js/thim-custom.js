(function ($) {
    ...
    var thim_startertheme = {
        ready: function () {
            ...
            var latestScroll = 0;
            if ($('#toolbar').length) {
                $('.header-overlay').css({top: $('#toolbar').outerHeight() + off_Top + 'px',});
            } else {
            ...
            $(window).scroll(function () {
                ...
                if (current > latestScroll && current > $elementToStick.outerHeight() + startFixed) {
                    if (!$header.hasClass('menu-hidden')) {
                        ...
                        $elementToStick.css({top: -$elementToStick.outerHeight(),});
                    }
                } else {
                    if ($header.hasClass('menu-hidden')) {
                        $header.removeClass('menu-hidden');
                        $elementToStick.css({top: dentalTop,});
                    }
                }
            ...
            $('.responsive .mobile-menu-container .navbar-nav>li.menu-item-has-children .icon-toggle, .mobile-menu-container .widget_nav_menu .menu-useful-links-container .menu>li.menu-item-has-children .icon-toggle').on('click', function () {
            ...
            $('.js-call-slick-col').each(function () {
                var data = [['responsive', 'array'], ['customdot', 'bool'], ['numofshow', 'number'], ['numofscroll', 'number'], ['fade', 'bool'], ['loopslide', 'bool'], ['autoscroll', 'bool'], ['speedauto', 'number'], ['verticalslide', 'bool'], ['verticalswipe', 'bool'], ['rtl', 'bool'], ['navfor', 'string'], ['animate', 'bool'], ['middlearrow', 'string']]
                var parameter = {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (value != null) {
                        if (data[i][1] === 'bool') {
                            ...
                            parameter[data[i][0]] = [[Number(strArray[0]), Number(strArray[1])], [Number(strArray[2]), Number(strArray[3])], [Number(strArray[4]), Number(strArray[5])], [Number(strArray[6]), Number(strArray[7])], [Number(strArray[8]), Number(strArray[9])]]
                        }
                    }
                }
                ...
                $(slideSlick).slick({
                    ...
                    responsive: [{
                        breakpoint: 1368,
                        settings: {slidesToShow: parameter.responsive[0][0], slidesToScroll: parameter.responsive[0][1]}
                    }, {
                        breakpoint: 1199,
                        settings: {slidesToShow: parameter.responsive[1][0], slidesToScroll: parameter.responsive[1][1]}
                    }, {
                        breakpoint: 991,
                        settings: {slidesToShow: parameter.responsive[2][0], slidesToScroll: parameter.responsive[2][1]}
                    }, {
                        breakpoint: 767,
                        settings: {slidesToShow: parameter.responsive[3][0], slidesToScroll: parameter.responsive[3][1]}
                    }, {
                        breakpoint: 575,
                        settings: {slidesToShow: parameter.responsive[4][0], slidesToScroll: parameter.responsive[4][1]}
                    }]
            ...
            $('#bp-popup-login #loginform').submit(function (event) {
                var elem = $('#bp-popup-login'), input_username = elem.find('#bp_login_name').val(),
            ...
            $('.login-popup form#loginform').submit(function (event) {
                var elem = $(this), input_username = elem.find('#bp_login_name, #bp_login_name_ac'),
            ...
            $('.login-popup form#registerform').submit(function (event) {
                var elem = $(this), input_username = elem.find('#user_login_register'),
                    input_email = elem.find('#user_email'), input_pass = elem.find('#password'),
            ...
            $('.login-popup form#lostpasswordform').submit(function (event) {
                var elem = $(this), input_username = elem.find('#user_login_lostpass');
                if (input_username.length > 0 && input_username.val() === '') {
                    input_username.addClass('invalid');
                    event.preventDefault();
                }
            ...
            try {
                ...
                $(".wrap-select2").each(function () {
                    ...
                    $(select).select2({minimumResultsForSearch: 20, dropdownParent: dropdown, theme: style,});
                });
            } catch (er) {
                console.log(er);
            }
            ...
            try {
                $('.popup-youtube, .popup-vimeo, .popup-gmaps').magnificPopup({
                    ...
                    iframe: {
                        patterns: {
                            youtube: {
                                index: 'youtube.com/', id: function (url) {
                                    var m = url.match(/[\\?\\&]v=([^\\?\\&]+)/);
